...
class Resources(object):
    ...
    def _resolve_all_privapps(self):
        ...
        if self._is_android_env:
            ...
            priv_app_dir = os.path.join(os.environ['ANDROID_PRODUCT_OUT'],
                                        'system/priv-app')
            if 'TARGET_COPY_OUT_PRODUCT' in os.environ == 'system/product':
                prod_priv_app_dir = os.path.join(os.environ['ANDROID_PRODUCT_OUT'], 'system/product/priv-app')
        ...
        return get_output('find %s -name "*.apk" && find %s -name "*.apk" && find %s  -name "*.apk" &&  find %s  -name "*.apk" &&  find %s  -name "*.apk" ' %(priv_app_dir,  prod_priv_app_dir, app_dir,prod_priv_app_dir, apps_dir)).split()
